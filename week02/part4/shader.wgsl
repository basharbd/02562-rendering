...
fn init_hit(r: Ray) -> HitInfo {
  return HitInfo(false, r.tmax, vec3f(0), vec3f(0), vec3f(0), vec3f(0), 0u, false);
}
...
fn intersect_scene(r: Ray) -> HitInfo {
  ...
  intersect_sphere(r, &h, vec3f(0.0, 0.5, 0.0), 0.3, sphere_id, vec3f(0.0, 0.0, 0.0));
...
fn shade_phong(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
  ...
  if (!is_occluded(shadow_ray)) {
    ...
    let specular = rho_s * L.Li * pow(r_dot_v, shininess);
    Lo += specular;
  }
  return Lo;
}
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
  switch ((*hit).shader) {
    ...
    case 2u: { return shade_phong(r, hit); }
