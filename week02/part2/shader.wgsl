...
fn init_hit(r: Ray) -> HitInfo {
  return HitInfo(false, r.tmax, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), 0u, false);
}
...
fn shade_mirror(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
  let R = reflect((*r).direction, hit.normal);
