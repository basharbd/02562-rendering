...
// -------------------------------------------------------------------------
// Intersection Logic (AABB, BSP, Primitive)
// -------------------------------------------------------------------------
...
fn intersect_sphere(r: Ray, hit: ptr<function, HitInfo>, center: vec3f, radius: f32) -> bool {
    ...
    var t = - bhalf - sqrt_disc;
    if (t < r.tmin || t > r.tmax) { t = - bhalf + sqrt_disc; }
    if (t < r.tmin || t > r.tmax) { return false; }
    hit.has_hit = true;
    hit.distance = t;
...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
    ...
    if (intersect_sphere(*r, hit, vec3f(130.0, 90.0, 250.0), 90.0)) { hit.ior1_over_ior2 = 1 / 1.5; hit.shader = 6u; r.tmax = hit.distance; }
...
fn sample_area_light(pos: vec3f, t: ptr<function, u32>) -> Light {
    ...
    let v0 = vInfo[face.x].position;
    let light_pos = v0 + (vInfo[face.y].position - v0) * rnd(t) + (vInfo[face.z].position - v0) * rnd(t);
    ...
    light.L_i = materials[meshFaces[l_idx].w].emission * f32(numLights);
    return light;
}
...
fn refract_fresnel(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
    ...
    if (costhetai < 0.0) { eta = 1 / hit.ior1_over_ior2; n = -hit.normal; costhetai = -costhetai; }
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
    switch hit.shader {
        ...
        case 6 { return refract_fresnel(r, hit, t); }
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
    ...
    for (var i = 0; i < 10; i++) {
        if (intersect_scene(&r, &hit)) { result += hit.throughput * shade(&r, &hit, &t); }
        else { result += hit.throughput * bgcolor.rgb; break; }
