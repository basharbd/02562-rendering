...
fn sample_cosine_weighted_direction(n: vec3f, t: ptr<function, u32>) -> vec3f {
    ...
    let phi = 2.0 * Pi * xi2;
    let s = spherical_direction(sin(theta), cos(theta), phi);
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, t: ptr<function, u32>) -> vec3f {
    ...
    if (rnd(t) < Pd) {
        ...
        r.direction = sample_cosine_weighted_direction(hit.normal, t);
        r.tmin = 0.001;
        r.tmax = 1.0e6;
        hit.emit = false;
        hit.has_hit = false;
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
    ...
    for (var p = 0u; p < subpixels; p++) {
        var r = Ray(uniforms.eye_point, normalize(uniforms.b1 * (uv.x + jitter[p].x + prog_jitter.x) + uniforms.b2 * (uv.y + jitter[p].y + prog_jitter.y) + uniforms.v * uniforms.camera_constant), 1e-4, 1e10);
