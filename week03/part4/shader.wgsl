...
@group(0) @binding(3) var<storage, read> jitter: array<vec2f>;
...
// Intersections
fn intersect_plane(r: Ray, hit: ptr<function, HitInfo>, p0: vec3f, onb: Onb) {
  ...
  if (t >= r.tmin && t <= r.tmax && t < (*hit).dist) {
    ...
    // Texture mapping with scale
    let plane_vec = (*hit).position - p0;
    let uu = dot(plane_vec, onb.tangent);
    let vv = dot(plane_vec, onb.binormal);
    let uv = vec2f(uu, vv) * u.tex_scale;
...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
  ...
  intersect_plane(*r, hit, vec3f(0.0), plane_onb);
  intersect_triangle(*r, hit, vec3f(-0.2, 0.1, 0.9), vec3f(0.2, 0.1, 0.9), vec3f(-0.2, 0.1, -0.1));
  intersect_sphere(*r, hit, vec3f(0.0, 0.5, 0.0), 0.3, u32(u.sphere_mat));
...
fn get_camera_ray(coords: vec2f) -> Ray {
  ...
  let b2 = cross(b1, v);
  let dir = normalize(coords.x * b1 + coords.y * b2 + u.cam_const * v);
  return Ray(eye, dir, 0.001, 1e32, 1.0);
}

@fragment
fn main_fs(@location(0) coords: vec2f) -> @location(0) vec4f {
  let uv_base = vec2f(coords.x * u.aspect * 0.5, coords.y * 0.5);
  
  var accum_color = vec3f(0.0);
  let samples = u32(u.subdivs * u.subdivs);
  ...
  return vec4f(pow(final_color, vec3f(1.0/u.gamma)), 1.0);
}